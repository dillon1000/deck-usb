...
// Enable display sync by default to prevent tearing.
    ...
    @autoreleasepool {
        try {
            ...
            for (int i=1; i<argc; ++i) {
                ...
                } else throw std::runtime_error("Usage: deck-usb [--list|--demo|--bench SECONDS] [--no-vsync] [--serial-usb] [--display-link] [--sensitivity N]");
            }
            if (!demo) {
